...
if (currentHours < 10) {hours.innerHTML = `0${currentHours} :`;}
else {hours.innerHTML = `${currentHours} : `}

let minutesCalculation = Math.floor((currentSeconds - (currentHours * 3600)) / 60);
let currentMinutes = minutesCalculation;
let minutes = document.querySelector("#minutes")

if (currentMinutes < 10) {minutes.innerHTML = `0${currentMinutes} :`;}
else{minutes.innerHTML = `${currentMinutes} :`}

let secondsCalculation = currentSeconds - (currentHours * 3600) - (currentMinutes * 60)
let seconds = document.querySelector("#seconds")

if (secondsCalculation < 10) {seconds.innerHTML = `0${secondsCalculation} `;}
else{seconds.innerHTML = `${secondsCalculation} `}
}
